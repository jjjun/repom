...
│   ├── completed/     # ✅ 解決済み Issue（連番付き: 001_*.md, 002_*.md）
│   ├── in_progress/   # 🚧 作業中の Issue
│   └── backlog/       # 📝 計画中・未着手の Issue
│
├── research/           # 🔬 技術調査と将来機能の検討
│   └── README.md      # 調査ガイドライン
│
├── ideas/              # 💡 機能アイデアと提案
│   └── README.md      # アイデア管理テンプレート
│
└── technical/          # 📖 技術詳細とAPIリファレンス
    └── README.md      # テクニカルドキュメント案内
```

### 🎯 各ディレクトリの用途

| ディレクトリ | 目的 | 内容 | ライフサイクル |
|-------------|------|------|--------------|
| `issue/` | 問題の追跡と解決 | バグ報告、実装課題、解決プロセス | backlog → in_progress → completed |
| `research/` | 技術調査 | 実現可能性調査、アプローチ比較、実装計画 | 将来機能の検討段階 |
| `ideas/` | 機能提案 | 新機能アイデア、改善提案、要望 | アイデア段階 |
| `technical/` | 技術詳細 | APIリファレンス、アーキテクチャ、使用ガイド | 活きたドキュメント |

### 🤖 AI エージェントとの協働

このプロジェクトでは、AI エージェントと効率的に協働できるよう設計されています：

- **問題報告**: 問題を伝えると、AI が自動的に Issue ファイルを作成し、一緒に解決作業を進めます
- **アイデア提案**: 機能アイデアを伝えると、AI がテンプレートに沿ってドキュメント化します
- **自動完了処理**: Issue が完了したことを伝えると、AI が自動的に `completed/` に移動し、インデックスを更新します

詳細なワークフローは `.github/copilot-instructions.md` に記載されています。

### 📚 詳細情報

各ディレクトリの詳細な使い方やテンプレートは、それぞれの `README.md` を参照してください：

- **Issue 管理**: `docs/issue/README.md`
- **調査ガイドライン**: `docs/research/README.md`
- **アイデアテンプレート**: `docs/ideas/README.md`
- **技術ドキュメント**: `docs/technical/README.md`
