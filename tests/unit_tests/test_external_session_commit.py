...
def test_internal_session_auto_commit(db_test):
    ...
    # インスタンスを保存
    instance = TestModel(name="internal_test")
...
def test_external_session_no_auto_commit(db_test):
    ...
    with _db_manager.get_sync_transaction() as session:
        ...
        instance = TestModel(name="external_test")
        saved = repo.save(instance)
        ...
        # 同じトランザクション内では見える
        stmt = select(TestModel).where(TestModel.name == "external_test")
...
def test_external_session_rollback_on_error(db_test):
    ...
    try:
        with _db_manager.get_sync_transaction() as session:
            ...
            # 正常なインスタンスを保存
            instance1 = TestModel(name="rollback_test_1")
            repo.save(instance1)
...
def test_internal_session_saves_with_commit(db_test):
    ...
    instances = [
        TestModel(name="batch_internal_1"),
        TestModel(name="batch_internal_2"),
        TestModel(name="batch_internal_3"),
    ]
...
def test_external_session_saves_no_commit(db_test):
    ...
    with _db_manager.get_sync_transaction() as session:
        ...
        instances = [
            TestModel(name="batch_external_1"),
            TestModel(name="batch_external_2"),
        ]

        repo.saves(instances)

        # トランザクション内では見える
        stmt = select(TestModel).where(TestModel.name.like("batch_external_%"))
        result = session.execute(stmt)
...
def test_internal_session_remove_with_commit(db_test):
    ...
    # テストデータを準備
    repo = TestRepository()
    instance = TestModel(name="remove_internal_test")
    saved = repo.save(instance)
    saved_id = saved.id
...
def test_external_session_remove_no_commit(db_test):
    ...
    # テストデータを準備
    prep_repo = TestRepository()
    instance = TestModel(name="remove_external_test")
    ...
    with _db_manager.get_sync_transaction() as session:
        ...
        # インスタンスを再取得して削除
        stmt = select(TestModel).where(TestModel.id == instance_id)
        result = session.execute(stmt)
        ...
        # トランザクション内では削除済み
        stmt2 = select(TestModel).where(TestModel.id == instance_id)
...
def test_multiple_repos_in_one_transaction(db_test):
    ...
    with _db_manager.get_sync_transaction() as session:
        ...
        # 複数のインスタンスを保存
        instance1 = TestModel(name="multi_1")
        instance2 = TestModel(name="multi_2")

        repo.save(instance1)
        repo.save(instance2)

        # トランザクション内では両方見える
        stmt = select(TestModel).where(TestModel.name.like("multi_%"))
        result = session.execute(stmt)
        found = result.scalars().all()
