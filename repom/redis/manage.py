...
def generate_redis_conf(password: str | None = None) -> str:
    ...
    password: when a password is configured, ``generate_docker_compose()``
    passes it to the container via the service environment and starts Redis
    with ``--requirepass`` instead of writing it here.
    """
