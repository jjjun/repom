...
with 文で使用することは**できません**。with 文で使う場合は `get_reusable_sync_transaction()` を使用してください。
...
**解決策**: 複数操作は `get_reusable_sync_transaction()` でラップする（パターン 2）
...
| 用途 | 推奨 API | 備考 |
|------|----------|------|
| FastAPI Depends（sync） | `get_db_session()` / `get_db_transaction()` | `Depends(...)` で利用 |
| sync の with 文（反復 transaction） | `get_reusable_sync_transaction()` | process 継続時に dispose しない |
| one-shot script（sync） | `get_standalone_sync_transaction()` | 終了時に engine を dispose |
| FastAPI Depends（async） | `get_async_db_session()` / `get_async_db_transaction()` + `get_lifespan_manager()` | lifespan で自動クリーンアップ |
| one-shot script（async） | `get_standalone_async_transaction()` | 終了時に async engine を dispose |
...
@app.post("/items")
def create_item(data: ItemCreate, session: Session = Depends(get_db_transaction)):
    item = Item(**data.dict())
    session.add(item)
    return item  # 自動 commit
```
...
#### 2. public context manager（with 文向け）

```python
from repom.database import get_reusable_sync_transaction, get_standalone_sync_transaction

# 複数 transaction を張る処理（task/worker/CLI）
with get_reusable_sync_transaction() as session:  # ✅ OK
    session.query(Model).all()

# one-shot スクリプト（終了時 dispose したい場合）
with get_standalone_sync_transaction() as session:  # ✅ OK
    session.add(item)
    # 自動コミット + 終了時 dispose
```
...
- ✅ with 文で使う場合は `get_reusable_sync_transaction()` を使用
- ✅ FastAPI では `Depends(get_db_session)` を使用
- ✅ CLI スクリプトでは `get_reusable_sync_transaction()` を使用
...
**方法2: with 文で使う場合は public context manager を使う**:
```python
# ✅ CLI やスクリプトの場合
from repom.database import get_reusable_sync_transaction

with get_reusable_sync_transaction() as session:  # ✅ OK（自動コミット）
    session.execute(...)
```
...
- with 文で使う場合は `get_reusable_sync_transaction()` を使用する

### TypeError: object AsyncSession can't be used in 'await' expression

**原因**: `AsyncSession` オブジェクト自体を `await` しています。

**間違った例**:
```python
session: AsyncSession = Depends(get_async_db_session)
await session  # ❌ AsyncSession 自体は await 不可
```

**正しい例**:
```python
session: AsyncSession = Depends(get_async_db_session)
result = await session.execute(...)  # ✅ I/O メソッドを await
```
...
**特徴**:
- ✅ `get_lifespan_manager()` で自動的にエンジンをクリーンアップ
- ✅ `Depends` パターンでシンプルに統合
- ✅ `dispose_engines()` の手動呼び出し不要
...
**特徴**:
- ✅ 自動的に `dispose_engines()` 相当のクリーンアップを行う
- ✅ プログラムが正常に終了する
- ✅ CLI ツール、バッチスクリプトに最適
...
**理由**: SQLAlchemy の非同期エンジンは接続プールとバックグラウンドタスクを保持し続けるため、明示的なエンジンクリーンアップなしでは終了しない場合があります。
...
**✅ 解決方法2: public API で手動管理する**:

```python
import asyncio
from repom.database import get_async_db_transaction, dispose_engines
...
| **pytest での非同期テスト** | fixture + `dispose_engines()` | fixture で管理 |
...
3. **複数操作はパターン 2** - `get_reusable_sync_transaction()` でラップする
...
**基本ルール（非同期版）**:
- FastAPI → `get_async_db_session()` + `get_lifespan_manager()`
- CLI/バッチ → `get_standalone_async_transaction()`
- 手動管理（内部検証用途）→ `get_async_db_transaction()` + `dispose_engines()`

**避けるべきこと**:
- ❌ Repository の `__init__` で `session is None` チェックして raise
- ❌ `__init__` で `get_db_session()` を直接呼ぶ
- ❌ パターン 1 で複数操作を実行
- ❌ スタンドアロンスクリプトでクリーンアップを忘れる
