...
repom keeps direct environment reads out of config dataclasses. Runtime
environment overrides live under `repom.config_hooks/` as small `apply_*`
helpers. For Redis, `repom.config_hooks.redis.apply_redis_env_overrides()`
reads `REDIS_PORT`, validates that it is an integer port between 1 and 65535,
and writes it to `config.redis.port`.

`repom.config` applies the Redis helper after `CONFIG_HOOK` returns, so
standalone repom usage keeps supporting `REDIS_PORT`. Downstream hooks can call
the helper explicitly at the end of their hook when they want the same ordering:
