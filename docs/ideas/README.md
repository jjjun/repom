...
docs/ideas/         → Initial concept and exploration
    ↓
docs/issues/active/  → Concrete implementation plan
    ↓
docs/issues/completed/ → Implementation complete
```
...
### 実装フェーズへ
実装に進められる段階になったら:
1. `docs/issues/active/XXX_[idea_name].md` を作成
...
アイデア運用に関する質問は以下を参照:
- `docs/issues/README.md` - Issue 管理フロー
