...
- Generated `uv.lock` and removed `poetry.lock`.
- Updated VSCode tasks, README/AGENTS/CLAUDE/Copilot instructions, docs/guides, and source/test command examples to use `uv`.
- Changed `repom_info` connection status symbols to ASCII `[OK]` / `[NG]` so the command does not fail on Windows cp932 output.
- Verified: `uv sync`, extras sync, `uv run pytest tests/unit_tests tests/behavior_tests` (875 passed, 9 skipped), `uv build`, `uv run repom_info`, and `uv run alembic current`.
